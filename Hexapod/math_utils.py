...
def rotate(src: point3d, theta):
    dest = point3d()
    theta = theta * math.pi / 180
    dest.x = src.x * math.cos(theta) - src.y * math.sin(theta)
    dest.y = src.x * math.sin(theta) + src.y * math.cos(theta)
    dest.z = src.z
    return dest

def rotateX(src: point3d, theta):
    dest = point3d()
    theta = theta * math.pi / 180
    dest.x = src.x
    dest.y = src.y * math.cos(theta) - src.z * math.sin(theta)
    dest.z = src.y * math.sin(theta) + src.z * math.cos(theta)
    return dest
